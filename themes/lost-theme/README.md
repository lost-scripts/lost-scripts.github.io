...
2. Clone this repository into the `themes` folder of your Hugo project:
	...
	```bash
	git clone https://github.com/lost-scripts/lost-theme.git themes/lost-theme
	```
...
Customize content by adding your files to the `content/` folder of your Hugo project.

For more details on how to structure your content and configuration, refer to the [Hugo Documentation](https://gohugo.io/documentation/).
...
1. Add Lost Theme as a Git Subtree to your project:

```bash
git subtree add --prefix=themes/lost-theme ../lost-theme.git main --squash
```


## Making Changes to the Theme

1. Edit files directly in `themes/lost-theme` within your Hugo project.

2. Remember to **always commit the changes** to your project before proceeding:

	```bash
	git add themes/lost-theme
...
After committing changes, push them to the central repository:

```bash
git subtree push --prefix=themes/lost-theme ../lost-theme.git main
```
...
```bash
git subtree pull --prefix=themes/lost-theme ../lost-theme.git main --squash
```


## Other

- **Bare Repository:**  The central repository for the theme must be configured as a "bare" repository to avoid conflicts when pushing from multiple projects:

	```bash
	git config --bool core.bare true
	cd ..
	mv lost-theme lost-theme.git
	```
